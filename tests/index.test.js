...
// Actual constants in src/index.js: MODEL_DIST_X = -270, MODEL_DIST_Y = 350
// moveCalibratedDot formula:
//   xpred = (coef_x * (rawX - 0.5) + x_intercept) * screen.width
//   ypred = (coef_y * rawY + y_intercept) * screen.height
...
describe('Pure Functions Tests', () => {
  ...
  describe('calculateCoefficients', () => {
    it('should calculate coefficients correctly for different distances', () => {
      ...
      expect(result60.coef_y).toBeCloseTo(5.8333, 4); // 350 / 60
    ...
    it('should handle negative distance', () => {
      ...
      expect(result.coef_y).toBeCloseTo(-11.6667, 4); // 350 / -30
    });
  ...
  describe('moveCalibratedDot', () => {
    ...
    it('should calculate screen coordinates correctly', () => {
      ...
      // coef_x = -270 / 60 = -4.5, coef_y = 350 / 60 ≈ 5.8333
      // xpred = (-4.5 * (0.5 - 0.5) + 0.2) * 1920 = 0.2 * 1920 = 384
      // ypred = (5.8333 * 0.5 + (-0.5)) * 1080 = (2.9167 - 0.5) * 1080 ≈ 2610
    ...
    it('should handle edge cases', () => {
      ...
      // xpred = (-4.5 * (0 - 0.5) + 0) * 1920 = 2.25 * 1920 = 4320
      expect(xpred1).toBe(4320);
      // ypred = (5.8333 * 0 + 0) * 1080 = 0
      expect(ypred1).toBe(0);

      // Test with rawX = 1, rawY = 1
      const [xpred2, ypred2] = moveCalibratedDot(1, 1, 60, 0, 0);
      // xpred = (-4.5 * (1 - 0.5) + 0) * 1920 = -2.25 * 1920 = -4320
      expect(xpred2).toBe(-4320);
      // ypred = (5.8333 * 1 + 0) * 1080 ≈ 6300
      expect(ypred2).toBeCloseTo(6300, 0);
    });

    it('should apply intercepts correctly', () => {
      const [xpred, ypred] = moveCalibratedDot(0.5, 0.5, 60, 0.1, 0.2);

      // xpred = (-4.5 * 0 + 0.1) * 1920 = 192
      // ypred = (5.8333 * 0.5 + 0.2) * 1080 = 3.1167 * 1080 ≈ 3366
    ...
    it('should handle different screen resolutions', () => {
      ...
      // xpred = 0.1 * 1366 = 136.6
      // ypred = (5.8333 * 0.5 + 0.2) * 768 = 3.1167 * 768 ≈ 2393.6
    ...
    it('should handle extreme gaze coordinates', () => {
      ...
      // xpred = (-4.5 * (0.001 - 0.5)) * 1920 = 2.2455 * 1920 ≈ 4311.36
      // ypred = (5.8333 * 0.001) * 1080 ≈ 6.3
      expect(xpred1).toBeCloseTo(4311, 0);
      expect(ypred1).toBeCloseTo(6.3, 1);

      // Test with values close to 1
      const [xpred2, ypred2] = moveCalibratedDot(0.999, 0.999, 60, 0, 0);
      expect(xpred2).toBeCloseTo(-4311, 0);
      expect(ypred2).toBeCloseTo(6294, 0);
    });
    ...
    it('should handle different distances', () => {
      ...
      // xpred = (-6.75 * 0 + 0.1) * 1920 = 192
      // ypred = (8.75 * 0.5 + 0.2) * 1080 = 4.575 * 1080 = 4941
      expect(xpred1).toBe(192);
      expect(ypred1).toBeCloseTo(4941, 0);

      // Test with distance 80cm
      const [xpred2, ypred2] = moveCalibratedDot(0.5, 0.5, 80, 0.1, 0.2);
      // coef_x = -270 / 80 = -3.375, coef_y = 350 / 80 = 4.375
      // xpred = 0.1 * 1920 = 192
      // ypred = (4.375 * 0.5 + 0.2) * 1080 = 2.3875 * 1080 ≈ 2578.5
      expect(xpred2).toBe(192);
      expect(ypred2).toBeCloseTo(2578.5, 0);
    });
  });
  ...
  describe('Mathematical edge cases', () => {
    ...
    it('should handle floating point precision', () => {
      ...
      expect(result.coef_y).toBeCloseTo(5.8333, 4);
    });
  });
